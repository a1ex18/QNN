...
# Parameters: `circuit` is circuit input value; `up_reg` is up reg input value; `n` is code length or generic size parameter; `with_swaps` is with swaps input value.
...
# Parameters: `circuit` is circuit input value; `q` is q input value; `ctl1` is ctl1 input value; `ctl2` is ctl2 input value; `aux` is aux input value; `a` is a input value; `N` is code length or generic size parameter; `n` is code length or generic size parameter.
...
# Parameters: `circuit` is circuit input value; `ctl` is ctl input value; `q` is q input value; `aux` is aux input value; `a` is a input value; `N` is code length or generic size parameter; `n` is code length or generic size parameter.
