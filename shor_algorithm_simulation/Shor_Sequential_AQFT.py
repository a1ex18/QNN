...
# Function: check_if_power - Helper routine for check if power logic.
# Parameters: `N` is code length or generic size parameter.
...
# Function: get_value_a - Helper routine for get value a logic.
# Parameters: `N` is code length or generic size parameter.
def get_value_a(N):
...
# Function: get_factors - Helper routine for get factors logic.
# Parameters: `x_value` is x value input value; `t_upper` is t upper input value; `N` is code length or generic size parameter; `a` is a input value.
...
# Function: egcd - Helper routine for egcd logic.
# Parameters: `a` is a input value; `b` is b input value.
def egcd(a, b):
    if a == 0:
        return (b, 0, 1)
    else:
        g, y, x = egcd(b % a, a)
        return (g, x - (b // a) * y, y)
# Function: modinv - Helper routine for modinv logic.
# Parameters: `a` is a input value; `m` is field/message parameter used by the algorithm.
def modinv(a, m):
    g, x, y = egcd(a, m)
    if g != 1:
...
# Function: create_QFT - Helper routine for create QFT logic.
# Parameters: `circuit` is circuit input value; `up_reg` is up reg input value; `n` is code length or generic size parameter; `with_swaps` is with swaps input value; `k` is message dimension or generic rank parameter.
...
# Function: create_inverse_QFT - Helper routine for create inverse QFT logic.
# Parameters: `circuit` is circuit input value; `up_reg` is up reg input value; `n` is code length or generic size parameter; `with_swaps` is with swaps input value; `k` is message dimension or generic rank parameter.
...
# Function: getAngle - Helper routine for getAngle logic.
# Parameters: `a` is a input value; `N` is code length or generic size parameter.
def getAngle(a, N):
...
# Function: getAngles - Helper routine for getAngles logic.
# Parameters: `a` is a input value; `N` is code length or generic size parameter.
def getAngles(a,N):
    s=bin(int(a))[2:].zfill(N) 
    angles=np.zeros([N])
...
# Function: ccphase - Helper routine for ccphase logic.
# Parameters: `circuit` is circuit input value; `angle` is angle input value; `ctl1` is ctl1 input value; `ctl2` is ctl2 input value; `tgt` is tgt input value.
...
# Function: phiADD - Helper routine for phiADD logic.
# Parameters: `circuit` is circuit input value; `q` is q input value; `a` is a input value; `N` is code length or generic size parameter; `inv` is inv input value.
...
# Function: cphiADD - Helper routine for cphiADD logic.
# Parameters: `circuit` is circuit input value; `q` is q input value; `ctl` is ctl input value; `a` is a input value; `n` is code length or generic size parameter; `inv` is inv input value.
...
# Function: ccphiADD - Helper routine for ccphiADD logic.
# Parameters: `circuit` is circuit input value; `q` is q input value; `ctl1` is ctl1 input value; `ctl2` is ctl2 input value; `a` is a input value; `n` is code length or generic size parameter; `inv` is inv input value.
...
# Function: ccphiADDmodN - Helper routine for ccphiADDmodN logic.
# Parameters: `circuit` is circuit input value; `q` is q input value; `ctl1` is ctl1 input value; `ctl2` is ctl2 input value; `aux` is aux input value; `a` is a input value; `N` is code length or generic size parameter; `n` is code length or generic size parameter; `kmax` is kmax input value.
...
# Function: ccphiADDmodN_inv - Helper routine for ccphiADDmodN inv logic.
# Parameters: `circuit` is circuit input value; `q` is q input value; `ctl1` is ctl1 input value; `ctl2` is ctl2 input value; `aux` is aux input value; `a` is a input value; `N` is code length or generic size parameter; `n` is code length or generic size parameter; `kmax` is kmax input value.
...
# Function: cMULTmodN - Helper routine for cMULTmodN logic.
# Parameters: `circuit` is circuit input value; `ctl` is ctl input value; `q` is q input value; `aux` is aux input value; `a` is a input value; `N` is code length or generic size parameter; `n` is code length or generic size parameter; `kmax` is kmax input value.
...
# Function: show_good_coef - Helper routine for show good coef logic.
# Parameters: `results` is results input value; `n` is code length or generic size parameter.
...
if __name__ == '__main__':
    ...
    # backend.set_options(device='GPU', method = "statevector_gpu")
